"""
domain A = real depth imgs (nyu dataset)
domain B = virtual depth imgs (princeton dataset)
"""
...
class CycleGAN():
    def __init__(self, trial, size):
        ...
        self.dataset_name = 'real_depth2virt_depth'
    ...
    def train(self, epochs, batch_size=128, save_interval=50):
        ...
        for epoch in range(epochs):
            ...
            imgs_A = self.data_loader.load_data(domain="real", batch_size=half_batch)
            imgs_B = self.data_loader.load_data(domain="virt", batch_size=half_batch)
            ...
            imgs_A = self.data_loader.load_data(domain="real", batch_size=batch_size)
            imgs_B = self.data_loader.load_data(domain="virt", batch_size=batch_size)
    ...
    def save_imgs(self, epoch):
        ...
        imgs_A = self.data_loader.load_data(domain="real", batch_size=1, is_testing=True)
        imgs_B = self.data_loader.load_data(domain="virt", batch_size=1, is_testing=True)
...
if __name__ == '__main__':
    ...
    size = 480  # 480 or 128
    epoch = 199999
    model_name = trial + '_' + str(size) + '_' + str(epoch)
    gan = CycleGAN(trial + '_' + str(size), size)

    if test:
        gan.model = load_neural((run_id+var_id), gan.loss, gan.opt)
        # gan.model.summary()


        n_samples = 2
        # n_tests = n_act*n_samples
        n_correct = 0
        img_folder = '../gest_img/clean_r/'
        morph_folder = '../gest_img/morph_r/'
        n_morph = 2
        morph_mult = 2
